...
def _pack_windows(ctx: _Ctx, path: str) -> None:
    ...
    if start is not None:
        windows.append((start, end, members))
    file_bytes = int(ctx.pack.manifest["nbytes"])                            # the packer pads the file to ALIGN
...
def _gemv(ctx: _Ctx, op: Op) -> None:
    ...
    choice = ctx.tuner.tune_gemv(info, t_c, op.attrs.get("epilogue"), stat is not None) if ctx.tuner is not None else None
    ...
    macros = dict(kernels.gemv_macros(info, t=t_c, rg=rg, epilogue=op.attrs.get("epilogue"), out_bf16=True, stat_out=stat_out is not None,
                                      norm=fuse_norm, round_before_residual=bool(op.attrs.get("round_residual"))), **ctx.t_macros(t_c, t_src))
    ...
    prm = ctx.params("gemv", kernels.gemv_params(info.n, info.n_blocks, n_sg, t_c, eps=eps,
...
def _gdn(ctx: _Ctx, op: Op) -> None:
    ...
    ctx.add(knorm, [(0, o_part, 0), (1, *ctx.buf(main)), (2, *ctx.windows[norm_w.name]), (3, *ctx.buf(out)), (4, prm, 0)],
...
def _accept_scan(ctx: _Ctx, op: Op) -> None:
    ...
    prm = ctx.params("accept_scan", kernels.accept_params(ctx.ring_capacity, ctx.eos))
    ctx.add(k, [(0, *ctx.buf(token)), (1, ctx.program.step_state, 0), (2, ctx.program.ring, 0), (3, prm, 0)], (1, 1, 1), (32, 1, 1), op.kind)


HANDLERS = {"embed": _embed, "rmsnorm_stat": _rmsnorm_stat, "norm_apply": _norm_apply_op, "gemv": _gemv, "lm_head": _gemv,
            "gqa_decode": _gqa, "gdn_mixer": _gdn, "argmax": _argmax, "sample": _sample,
...
def emit_program(g: Graph, *, pack: PackFile, profile: Profile, t: Optional[int] = None, dynamic_t: bool = False,
                 layout: Optional[StepStateLayout] = None, eos: int = -1, ring_capacity: int = 4096, tg: int = 384, tuner: Any = None,
                 tail: Optional[str] = "advance", token: Optional[Value] = None) -> Program:
    ...
    at run time (kernels compiled at ``t_max``). ``tail="advance"`` appends the step's advance on ``token`` (the
    ...
    ctx = _Ctx(program, pack, layout, t, 12 * profile.gpu_cores * profile.threadgroups_per_core, tg, values=g.values, dynamic_t=dynamic_t,
               tuner=tuner, eos=eos, ring_capacity=ring_capacity)
...
def compile_program(model: Model, pack: PackFile, profile: Profile, *, t: Optional[int] = None, eos: int = -1, ring_capacity: int = 4096,
    ...
    """Lower ``model``, run the ``passes`` and emit its step program (see :func:`emit_program`)."""
    g = Graph("step")
    token = model.lower(g)
    g.check()
    for p in passes:
