...
def run(dev, heads, kv, d, ctx, t, chunk, rb, reps):
    ...
    params = kernels.gqa_params(heads=heads, kv_heads=kv, t_active=t, position=ctx, n_sg=n_sg, q_off=0, gate_off=hd, k_off=2 * hd,
                                v_off=2 * hd + kd, in_stride=n1, out_stride=hd, ctx_max=ctx_max, eps=1e-6, scaling=d ** -0.5,
                                has_gate=True, n_chunks_max=n_chunks_max, rows_max=rows_max)
    ...
    return {"heads": heads, "kv": kv, "d": d, "ctx": ctx, "t": t, "chunk": chunk, "rb": rb, "n_blocks": kv * (-(-(ctx + t) // chunk)) * (-(-rows_max // rb)),
...
def main(argv=None) -> int:
    ...
    dev = nt.Device()
    info = dev.info()
    print(f"# {info.name}, {info.gpu_cores} cores; {time.strftime('%Y-%m-%d %H:%M')}; heads={a.heads} kv={a.kv} d={a.d} chunk={a.chunk}; min of {a.reps}")
    for ctx in (int(x) for x in a.ctx.split(",")):
        for t in (int(x) for x in a.t.split(",")):
            r = run(dev, a.heads, a.kv, a.d, ctx, t, a.chunk, a.rb, a.reps)
